...
# --- "종목분석2" 프로토타입 -------------------------------------------------
# 사용자가 제시한 LLM 시스템 프롬프트(입력 JSON -> 분석 지침 5개 -> 출력 JSON
# 스키마)를 실제 LLM API 호출 없이 규칙 기반으로 재현한다(사용자가 명시적으로
# 선택한 구현 방식 — API 키/비용 없이 같은 입출력 스키마의 결과를 낸다). "종목
# 분석" 탭과 DART 데이터 소스는 같지만, 이쪽은 프롬프트가 요구하는 스키마(요약
# 표 / 가설 수준 해석 / 동종업계 비교 / 긍정·부정 신호 3개씩 / 종합 코멘트 /
# 확인 필요 사항)를 그대로 따르는 별도 프로토타입이다.

def _compute_stock_metrics_v2(ticker):
    """company/peer 공통으로 쓰는 지표 묶음 하나를 계산한다. 매출·영업이익의
    YoY/QoQ는 최근 20개 분기(fetch_dart_quarterly_financials_5y_cached)에서
    바로 앞 분기·4개 분기 전을 비교해서 구하고, 순이익/부채비율/ROE는 최근
    사업연도(fetch_dart_balance_ratios_5y_cached)에서 가져온다 — 둘 다 "종목
    분석" 탭에서 이미 쓰는 것과 같은 소스라 사업보고서/분기보고서가 아직 안
    나온 경우를 포함해 동작이 검증돼 있다. 데이터가 없으면 해당 값은 None."""
    quarterly_items, _ = fetch_dart_quarterly_financials_5y_cached(ticker)
    latest_q = quarterly_items[-1] if quarterly_items else None
    qoq_prev = quarterly_items[-2] if len(quarterly_items) >= 2 else None
    yoy_prev = quarterly_items[-5] if len(quarterly_items) >= 5 else None

    revenue = latest_q['revenue'] if latest_q else None
    operating_income = latest_q['operating_income'] if latest_q else None
    revenue_yoy = _growth_pct(revenue, yoy_prev['revenue']) if latest_q and yoy_prev else None
    revenue_qoq = _growth_pct(revenue, qoq_prev['revenue']) if latest_q and qoq_prev else None
    oi_yoy = _growth_pct(operating_income, yoy_prev['operating_income']) if latest_q and yoy_prev else None

    balance_ratios_by_year = fetch_dart_balance_ratios_5y_cached(ticker)
    net_income = debt_ratio = roe = None
    if balance_ratios_by_year:
        latest_ratios = balance_ratios_by_year[max(balance_ratios_by_year)]
        net_income = latest_ratios.get('net_income')
        debt_ratio = latest_ratios.get('debt_ratio')
        roe = latest_ratios.get('roe')

    period_label = f"{latest_q['year']}Q{latest_q['quarter']}" if latest_q else None
    return {
        'period': period_label,
        'revenue': revenue, 'revenue_yoy': revenue_yoy, 'revenue_qoq': revenue_qoq,
        'operating_income': operating_income, 'oi_yoy': oi_yoy,
        'net_income': net_income, 'debt_ratio': debt_ratio, 'roe': roe,
    }


@st.cache_data(ttl=PER_TTL_SEC)
def fetch_market_cap_series_v2(market):
    """"종목분석2"의 동종업계 비교용 — 시가총액이 비슷한 순으로 경쟁사를 고르려고
    시장 전체 시가총액을 한 번에 받는다(날짜 재시도 규칙은 fetch_per_universe와
    동일). 반환: 티커를 인덱스로 하는 시가총액(원) Series — 실패하면 빈 Series."""
    for delta in range(PER_LOOKBACK_DAYS):
        d = (datetime.now() - pd.Timedelta(days=delta)).strftime('%Y%m%d')
        try:
            df = pykrx_stock.get_market_cap_by_ticker(d, market=market)
        except Exception:
            continue
...
def _pick_industry_peers_v2(ticker, market, industry_name, n=2):
    """같은 업종(industry_name) 내에서 시가총액이 가장 비슷한 다른 종목 n개를
    고른다. 단순히 종목코드순으로 뽑으면(예전 방식) "삼성전자 vs 두산"처럼
    체급이 전혀 다른 회사가 "경쟁사"로 잡히는 문제가 있어, 규모가 가까운
    순으로 정렬해 실질적으로 비교할 만한 동종업계 종목을 고른다. 시가총액을
    못 받아오면 기존처럼 종목코드순으로 대체한다."""
    if not industry_name or market not in ('KOSPI', 'KOSDAQ'):
        return []
    try:
        universe = fetch_per_universe(market)
    except Exception:
        return []
    if universe.empty or '업종명' not in universe.columns:
        return []
    peers = universe[(universe['업종명'] == industry_name) & (universe.index != ticker)]
    # 우선주 종목코드는 국내 관행상 마지막 자리가 0이 아니다(예: 삼성전자 005930 vs
    # 삼성전자우 005935) — 그대로 두면 "경쟁사"가 아니라 같은 회사의 우선주가
    # 뽑히는 경우가 있어 제외한다.
    peers = peers[peers.index.str.endswith('0')]
    if peers.empty:
        return []

    try:
        market_caps = fetch_market_cap_series_v2(market)
    except Exception:
        market_caps = pd.Series(dtype=float)
    own_cap = market_caps.get(ticker) if not market_caps.empty else None
    if own_cap:
        peer_caps = market_caps.reindex(peers.index).dropna()
        if not peer_caps.empty:
            return list((peer_caps - own_cap).abs().sort_values().head(n).index)
    return list(peers.sort_index().head(n).index)


def _fmt_v2(value, unit='', decimals=0, signed=False):
    if value is None:
        return '-'
    sign = '+' if signed else ''
    return f'{value:{sign},.{decimals}f}{unit}'


def _summarize_metrics_v2(metrics):
    """분석 지침 1) 핵심 지표를 표로 요약하고 쉬운 말로 설명."""
    rows = [
        ('매출액', _fmt_v2(metrics['revenue'], '억원'), '이번 분기에 벌어들인 전체 매출입니다.'),
        ('매출 YoY', _fmt_v2(metrics['revenue_yoy'], '%', 1, True), '작년 같은 분기 대비 매출 증감률입니다.'),
        ('매출 QoQ', _fmt_v2(metrics['revenue_qoq'], '%', 1, True), '바로 전 분기 대비 매출 증감률입니다.'),
        ('영업이익', _fmt_v2(metrics['operating_income'], '억원'), '매출에서 원가·판관비를 뺀, 본업으로 남긴 이익입니다.'),
        ('영업이익 YoY', _fmt_v2(metrics['oi_yoy'], '%', 1, True), '작년 같은 분기 대비 영업이익 증감률입니다.'),
        ('순이익', _fmt_v2(metrics['net_income'], '억원'), '세금 등을 모두 뗀 뒤 최종적으로 남긴 이익입니다(최근 사업연도 기준).'),
        ('부채비율', _fmt_v2(metrics['debt_ratio'], '%'), '자기자본 대비 빚의 비율 — 100% 미만이면 상대적으로 안정적입니다.'),
        ('ROE', _fmt_v2(metrics['roe'], '%', 1), '자기자본으로 얼마나 효율적으로 이익을 냈는지 보여주는 비율입니다.'),
    ]
    return [{'지표': label, '값': value, '설명': desc} for label, value, desc in rows]


def _interpret_yoy_qoq_v2(metrics):
    """분석 지침 2) 전년/전분기 대비 변화를 "가설 수준"으로만 해석 — 단정하지 않는다."""
    lines = []
    ry, rq, oy = metrics['revenue_yoy'], metrics['revenue_qoq'], metrics['oi_yoy']
    if ry is not None:
        direction = '늘었습니다' if ry > 0 else ('줄었습니다' if ry < 0 else '비슷한 수준입니다')
        lines.append(f'매출은 전년 동기 대비 {ry:+.1f}% {direction}.')
    if rq is not None:
        direction = '늘었습니다' if rq > 0 else ('줄었습니다' if rq < 0 else '비슷한 수준입니다')
        lines.append(f'전분기 대비로는 {rq:+.1f}% {direction}.')
    if ry is not None and oy is not None:
        if ry > 0 and oy < 0:
            lines.append(
                '매출은 늘었는데 영업이익은 줄어든 흐름이라, 원가나 판관비 부담이 커졌을 '
                '가능성을 가설 수준에서 짚어볼 수 있습니다(단정할 근거는 아닙니다).'
            )
        elif ry < 0 and oy > 0:
            lines.append(
                '매출은 줄었는데 영업이익은 늘어난 흐름이라, 비용 절감이나 일회성 이익이 '
                '반영됐을 가능성을 가설 수준에서 짚어볼 수 있습니다(단정할 근거는 아닙니다).'
            )
        elif ry > 0 and oy > 0:
            lines.append('매출과 영업이익이 함께 늘어난 흐름으로, 본업 성장이 이익으로 이어지고 있을 가능성이 있습니다(가설 수준).')
        elif ry < 0 and oy < 0:
            lines.append(
                '매출과 영업이익이 함께 줄어든 흐름으로, 업황 둔화나 수요 위축 가능성을 '
                '가설 수준에서 짚어볼 수 있습니다(단정할 근거는 아닙니다).'
            )
    if not lines:
        return '전년/전분기 비교에 필요한 분기 데이터가 부족해 해석할 수 없습니다.'
    return ' '.join(lines)


def _interpret_peer_comparison_v2(metrics, peer_metrics):
    """분석 지침 3) peer_metrics와 비교한 상대적 위치."""
    if not peer_metrics:
        return '같은 업종 내 비교 가능한 경쟁사 데이터를 찾지 못해 상대 비교를 생략합니다.'
    lines = []
    for key, label, higher_is_better in (
        ('roe', 'ROE', True), ('debt_ratio', '부채비율', False), ('revenue_yoy', '매출 성장률(YoY)', True),
    ):
        own = metrics.get(key)
        peer_vals = [p[key] for p in peer_metrics if p.get(key) is not None]
        if own is None or not peer_vals:
            continue
        peer_avg = sum(peer_vals) / len(peer_vals)
        if own == peer_avg:
            cmp_word = '비슷해'
        elif (own > peer_avg) == higher_is_better:
            cmp_word = '높아' if own > peer_avg else '낮아'
        else:
            cmp_word = '높아' if own > peer_avg else '낮아'
        lines.append(f'{label}은 {own:.1f}%로 경쟁사 평균({peer_avg:.1f}%)보다 {cmp_word} 보입니다.')
    peer_names = ', '.join(p['company'] for p in peer_metrics)
    if not lines:
        return f'비교 대상: {peer_names}. 경쟁사 데이터는 있으나 비교 가능한 공통 지표가 부족합니다.'
    return f'비교 대상: {peer_names}. ' + ' '.join(lines)


_SIGNAL_FALLBACK_V2 = (
    '이번 조회 범위에서는 이 방향의 뚜렷한 신호를 추가로 찾지 못했습니다 — '
    '데이터가 제한적이니 원문 재무제표로 별도 확인을 권합니다.'
)


def _build_signals_v2(metrics, peer_metrics):
    """분석 지침 4) 긍정 3개·부정 3개를 "근거 지표"와 함께 제시. 후보를 넉넉히
    만든 뒤 우선순위대로 최대 3개씩 뽑고, 그래도 모자라면(데이터가 많이 비어
    있는 경우) 숫자를 새로 지어내지 않고 정해진 안내 문구로 채운다."""
    peer_avg = {}
    for key in ('roe', 'debt_ratio', 'revenue_yoy'):
        vals = [p[key] for p in peer_metrics if p.get(key) is not None]
        if vals:
            peer_avg[key] = sum(vals) / len(vals)

    pos, neg = [], []
    roe, debt_ratio = metrics.get('roe'), metrics.get('debt_ratio')
    ry, rq, oy = metrics.get('revenue_yoy'), metrics.get('revenue_qoq'), metrics.get('oi_yoy')
    net_income = metrics.get('net_income')

    if roe is not None:
        if roe >= 10:
            pos.append(f'ROE {roe:.1f}%로 자기자본 대비 수익성이 양호합니다(근거 지표: ROE).')
        elif roe < 3:
            neg.append(f'ROE {roe:.1f}%로 자기자본 대비 수익성이 낮은 편입니다(근거 지표: ROE).')

    if debt_ratio is not None:
        if debt_ratio < 100:
            pos.append(f'부채비율 {debt_ratio:.0f}%로 재무구조가 안정적인 편입니다(근거 지표: 부채비율).')
        elif debt_ratio > 200:
            neg.append(f'부채비율 {debt_ratio:.0f}%로 재무 부담이 큰 편입니다(근거 지표: 부채비율).')

    if ry is not None and oy is not None:
        if ry > 0 and oy > 0:
            pos.append(f'매출 YoY {ry:+.1f}%, 영업이익 YoY {oy:+.1f}%로 동반 성장했습니다(근거 지표: 매출·영업이익 YoY).')
        elif ry > 0 and oy < 0:
            neg.append(f'매출은 YoY {ry:+.1f}% 늘었지만 영업이익은 {oy:+.1f}%로 줄어 마진이 악화된 흐름입니다(근거 지표: 매출·영업이익 YoY).')
        elif ry < 0 and oy < 0:
            neg.append(f'매출 YoY {ry:+.1f}%, 영업이익 YoY {oy:+.1f}%로 동반 역성장했습니다(근거 지표: 매출·영업이익 YoY).')

    if rq is not None:
        if rq > 0:
            pos.append(f'매출이 전분기 대비 {rq:+.1f}% 늘어 단기 모멘텀은 나쁘지 않습니다(근거 지표: 매출 QoQ).')
        elif rq < 0:
            neg.append(f'매출이 전분기 대비 {rq:+.1f}% 줄어 단기 모멘텀이 둔화된 모습입니다(근거 지표: 매출 QoQ).')

    if roe is not None and 'roe' in peer_avg and roe != peer_avg['roe']:
        if roe > peer_avg['roe']:
            pos.append(f'ROE {roe:.1f}%가 경쟁사 평균 {peer_avg["roe"]:.1f}%보다 높습니다(근거 지표: ROE, 경쟁사 비교).')
        else:
            neg.append(f'ROE {roe:.1f}%가 경쟁사 평균 {peer_avg["roe"]:.1f}%보다 낮습니다(근거 지표: ROE, 경쟁사 비교).')

    if debt_ratio is not None and 'debt_ratio' in peer_avg and debt_ratio != peer_avg['debt_ratio']:
        if debt_ratio < peer_avg['debt_ratio']:
            pos.append(f'부채비율 {debt_ratio:.0f}%가 경쟁사 평균 {peer_avg["debt_ratio"]:.0f}%보다 낮습니다(근거 지표: 부채비율, 경쟁사 비교).')
        else:
            neg.append(f'부채비율 {debt_ratio:.0f}%가 경쟁사 평균 {peer_avg["debt_ratio"]:.0f}%보다 높습니다(근거 지표: 부채비율, 경쟁사 비교).')

    if net_income is not None:
        if net_income > 0:
            pos.append(f'최근 사업연도 순이익이 {net_income:,.0f}억원으로 흑자입니다(근거 지표: 순이익).')
        else:
            neg.append(f'최근 사업연도 순이익이 {net_income:,.0f}억원으로 적자입니다(근거 지표: 순이익).')

    pos = (pos + [_SIGNAL_FALLBACK_V2] * 3)[:3]
    neg = (neg + [_SIGNAL_FALLBACK_V2] * 3)[:3]
    return pos, neg


def _build_overall_view_v2(company, pos, neg):
    """분석 지침 5) "매수 추천"이 아니라 강점/리스크/확인 필요 사항으로 마무리."""
    real_pos = [p for p in pos if p != _SIGNAL_FALLBACK_V2]
    real_neg = [n for n in neg if n != _SIGNAL_FALLBACK_V2]
    return (
        f'{company}은(는) 위에서 짚은 대로 강점({len(real_pos)}건)과 리스크({len(real_neg)}건)가 '
        '함께 확인됩니다. 이 내용은 매수·매도를 권하는 것이 아니라, 투자자가 스스로 판단할 때 '
        '참고할 수 있는 강점·리스크·확인 필요 사항을 정리한 것입니다.'
    )


def _build_needs_verification_v2(metrics, peer_metrics, data_asof):
    """분석 지침에는 명시돼 있지 않지만 출력 스키마가 요구하는 needs_verification —
    이번 조회에서 비어 있던 데이터와, data_asof 이후 최신 여부를 항상 포함한다."""
    items = []
    if metrics.get('net_income') is None:
        items.append('순이익 데이터를 찾지 못해 확인이 필요합니다.')
    if metrics.get('debt_ratio') is None or metrics.get('roe') is None:
        items.append('부채비율·ROE 중 일부 데이터를 찾지 못해 확인이 필요합니다.')
    if not peer_metrics:
        items.append('같은 업종 내 비교 가능한 경쟁사 데이터를 찾지 못했습니다.')
    items.append(f'본 자료는 {data_asof} 기준이며, 그 이후 공시·실적 발표가 있었는지 원문에서 재확인하세요.')
    return items


def render_stock_analysis_v2():
    """"종목분석2" 탭 본체 — 검색 UI는 "종목 분석" 탭과 같은 패턴을 쓰고, 계산된
    지표를 프롬프트가 정의한 입력 JSON으로 묶은 뒤, 그 JSON만 보고 해석하는
    규칙 기반 함수들을 거쳐 출력 JSON 스키마 그대로 화면에 렌더링한다."""
    st.subheader('종목분석2 (프로토타입)')
    st.caption(
        '사용자가 제시한 LLM 분석 프롬프트의 입력·출력 스키마를 그대로 따르되, '
        'API 키/비용 없이 규칙 기반으로 해석 문장을 생성합니다.'
    )

    listing = load_krx_listing()
    if listing.empty:
        st.warning('종목 목록을 불러오지 못했습니다. 종목코드를 직접 입력해 주세요.')
        ticker = st.text_input('종목코드 (6자리)', key='v2_ticker_manual').strip()
        if not ticker:
            return
        name, market = ticker, None
    else:
    ...
    with st.spinner(f'{name} 데이터를 불러오는 중...'):
        metrics = _compute_stock_metrics_v2(ticker)
        ...
        if market in ('KOSPI', 'KOSDAQ'):
            try:
                universe = fetch_per_universe(market)
                if not universe.empty and ticker in universe.index:
                    industry_name = universe.loc[ticker, '업종명']
            except Exception:
                industry_name = None

        peer_tickers = _pick_industry_peers_v2(ticker, market, industry_name, n=2)
        peer_metrics = []
        for peer_ticker in peer_tickers:
            peer_name = (
                listing.loc[peer_ticker, 'Name'] if not listing.empty and peer_ticker in listing.index else peer_ticker
            )
            pm = _compute_stock_metrics_v2(peer_ticker)
            pm.pop('period', None)
            pm['company'] = peer_name
            peer_metrics.append(pm)

    data_asof = datetime.now().strftime('%Y-%m-%d')
    period_label = metrics.get('period') or '-'
    payload = {
        'company': name, 'period': period_label,
        'metrics': {k: v for k, v in metrics.items() if k != 'period'},
        'peer_metrics': peer_metrics,
        'data_asof': data_asof, 'data_source': 'DART 공시',
    }

    positive_signals, negative_signals = _build_signals_v2(metrics, peer_metrics)
    result = {
        'summary': _summarize_metrics_v2(metrics),
        'yoy_qoq_interpretation': _interpret_yoy_qoq_v2(metrics),
        'peer_comparison': _interpret_peer_comparison_v2(metrics, peer_metrics),
        'positive_signals': positive_signals,
        'negative_signals': negative_signals,
        'overall_view': _build_overall_view_v2(name, positive_signals, negative_signals),
        'needs_verification': _build_needs_verification_v2(metrics, peer_metrics, data_asof),
        'disclaimer': '본 내용은 투자 자문이 아니며 참고용입니다.',
    }

    st.markdown(f'##### {name} ({ticker}) · {period_label} 기준')
    industry_caption = f'업종: {industry_name} · ' if industry_name else ''
    st.caption(f'{industry_caption}데이터 기준일: {data_asof} · 출처: DART 공시')

    st.markdown('###### 1. 핵심 지표 요약')
    st.dataframe(pd.DataFrame(result['summary']), use_container_width=True, hide_index=True)

    st.markdown('###### 2. 전년/전분기 대비 해석 (가설 수준)')
    st.markdown(result['yoy_qoq_interpretation'])

    st.markdown('###### 3. 동종업계 비교')
    st.markdown(result['peer_comparison'])
    if peer_metrics:
        peer_table = [{'회사': name, **payload['metrics']}]
        peer_table += [{'회사': p['company'], **{k: v for k, v in p.items() if k != 'company'}} for p in peer_metrics]
        st.dataframe(pd.DataFrame(peer_table), use_container_width=True, hide_index=True)
    ...
    st.markdown('###### 5. 종합 코멘트')
    st.info(result['overall_view'])
    st.markdown('**확인이 필요한 사항**')
    for item in result['needs_verification']:
        st.markdown(f'- {item}')
    st.caption(result['disclaimer'])

    with st.expander('입력 JSON (LLM 프롬프트에 들어갈 형태)'):
        st.json(payload)
    with st.expander('출력 JSON (LLM 응답 스키마)'):
        st.json(result)
...
def main():
    ...
    tab_market, tab_portfolio, tab_fx_news, tab_rs, tab_opinion, tab_opinion_v2 = st.tabs(
        ['전체 시장현황', '내 계좌 포트폴리오', '환율 및 뉴스', 'RS 비교', '종목 분석', '종목분석2']
    )
