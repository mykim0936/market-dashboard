...
# "포트폴리오 성과 추이"에서 고를 수 있는 비교 지수 — (소스, 코드)는 MARKET_SERIES와 같은 규칙.
RS_BENCHMARKS = {
    '코스피': ('pykrx', '1001'),
    '코스닥': ('pykrx', '2001'),
...
def load_series_live(source, code, start_dt):
    ...
    start_dt 는 정확한 시작 시점(datetime/Timestamp) — "포트폴리오 성과 추이"처럼
    "1개월 전부터" 같은 정확한 기간이 필요한 경우도 있어 연 단위가 아니라 날짜를 직접 받는다."""
    if source == 'pykrx':
        try:
...
# 포트폴리오 성과 추이에서 고를 수 있는 기간 — 각각 "지금부터 이만큼 전"의 시작일을 계산하는 함수.
...
def fetch_stock_series(ticker, start_dt):
    """보유/검색 종목 개별 시계열(FDR, 지수 라이브 조회와 별개 경로)."""
...
def render_getting_started():
    ...
    with st.expander('처음 오셨나요? — 탭 안내와 숫자 읽는 법', expanded=False):
        st.markdown(
            ...
            '- **내 계좌 포트폴리오** — 내가 가진 종목의 손익, 비중 쏠림, 위험 신호, 지수 대비 성과를 봅니다. '
            '*자기 계좌를 점검하는 곳입니다.*\n'
